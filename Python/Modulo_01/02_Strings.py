"""
    ...
    ou variáveis.
        ...
        Strings nada mais é do que textos, dados com uso de caracteres, onde o mesmo 
    pode ser números, caso este seja tratado como string, não pode ser realizado 
    calculos com ele.

        O uso de números como string serve, por exemplo, para o armazenamento de 
    CPF de clientes já que, ao ser armazenado, string deixará todos os números, 
    até mesmo espaços vazios nas linhas, porem, se um CPf foi armazenado como um 
    tipo primitivo de número, e o mesmo iniciar com zero, a linguagem eliminará o 
    zero deixando o CPF do cliente incompleto

        O código de String em python é str, e sempre deve estar entre aspas simples, 
    a linguagem aceita o uso de aspas duplas porem, por convenção, se usa aspas 
    simples.

        Neste texto que está lendo agora, ele está entre três aspas duplas, também 
    pode ser feito em três aspas simples, este formato também é uma string mas a 
    convenção é a ser usado para realizar a documentação, explicando o programa ou 
    uma função em específico.
